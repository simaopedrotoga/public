...
for size in range(10**5, 12*10**5+1, 10**5):
    ...
    # in list
    tl_stmt = "10*10**5 in c"                            # the list gets bigger
    tl_setup = "c = list(range(" + str(size) + "))"      # the operation
    pl = timeit.timeit(tl_stmt, tl_setup, number=10**3)  # timeit for 1000 pops

    # in dict
    ts_stmt = "10*10**5 in s"                            # the set gets bigger
    ts_setup = "s = set(range(" + str(size) + "))"       # the operation
